...
botsession = os.getenv('BOT_SESSION')
...
async def process_file(event):
    ...
    await event.respond("The file is being downloaded, please wait meanwhile.")
    file = await message.download_media(progress_callback=download_callback)
    ...
    await event.respond(f"Audio tracks found in the file:\n{audio_tracks_list} \ntype /select followed by the numbers of"
                        ...
                        f"Please wait while it's working")
...
async def handle_select(event):
    ...
    upload_result = await thebot.send_file(event.chat_id, file=output_file, progress_callback=upload_callback)
